#!/bin/bash
# ============================================================================
# RAG System v47 - Query Setup (ALL FEATURES IMPLEMENTED)
# ============================================================================
...
clear
echo "============================================================================"
echo "   RAG System v47 - Query Setup (Full Features)"
echo "============================================================================"
echo ""
...
def get_embedding(text, timeout=60, is_query=True):
    """Get embedding vector - v47: FastEmbed with Ollama fallback"""
    global _debug_info
    config = get_config()
    ...
    # v47: Try FastEmbed first
    fastembed_enabled = os.environ.get("FEATURE_FASTEMBED_ENABLED", "true") == "true"
    
    if fastembed_enabled:
        try:
            from fastembed_helper import get_embedding as fe_embed, is_available
            if is_available():
                embedding = fe_embed(text[:8000], is_query=is_query)
                if embedding:
...
def get_embedding(text, is_query=True):
    """Get embedding - v47: FastEmbed with Ollama fallback"""
    # v47: Try FastEmbed first
    fastembed_enabled = os.environ.get("FEATURE_FASTEMBED_ENABLED", "true") == "true"
...
echo ""
echo "============================================================================"
echo "   Query Setup Complete! (v47 - FastEmbed + Quality Feedback)"
echo "============================================================================"
echo ""
...
echo ""
echo "v47 FEATURES (FastEmbed):"
echo "  + ONNX-based embeddings (CPU optimized)"
echo "  + Model: BAAI/bge-small-en-v1.5 (or bge-base)"
echo "  + Offline after first download"
echo "  + Ollama fallback if FastEmbed unavailable"
echo ""
